...
def build_ground_truth(leakages: pd.DataFrame) -> pd.Series:
    """GT: ANY node >0.01 → leak=1 (time-series magnitudes)."""
    print(f"  🏗️ Raw leaks shape: {leakages.shape}")
    leakages.index = pd.to_datetime(leakages.index, errors='coerce')
    
    # Resample to match SCADA freq (5min)
    leakages_resampled = leakages.resample('5min').mean().fillna(0)
    
    # Binary: ANY col > threshold
    gt = (leakages_resampled > 0.01).any(axis=1).astype(int)
    
    print(f"  ✅ GT: {gt.sum():,} leak steps ({gt.mean():.1%} rate)")
    return gt